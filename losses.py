...
def humidity_constraint_loss(y_pred, target, rh, th: Thresholds):
    if rh is None:
        return _safe_zero_like(y_pred)
    # NOTE: your original used relu with sign-specific terms; keeping spirit but safer:
...
def lagged_effects_constraint_loss(y_pred, target, temp, rain, temperature_lag: int = 2, rainfall_lag: int = 16):
...
class CustomLoss(QuantileLoss):
    ...
    IMPORTANT: PyTorch Forecasting will call loss(y_pred, target).
    If you want to pass extra variables (temp/rain/etc), you typically
    need them inside the network output or batch and override training_step.
    """
